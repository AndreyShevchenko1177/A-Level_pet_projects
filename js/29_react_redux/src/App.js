...
function createStore(reducer) {
    ...
    return {
        ...
        subscribe(cb) {
            cbs.push(cb);
            return () => { cbs = cbs.filter((someElement) => someElement !== cb)}
        },
    };
}
...
const store = createStore(
    (state = {}, { type, status, payload, error, name }) =>
        type === "PROMISE" 
...
const ReduxViewer = () => {
    ...
    return (
        <pre>
            <button onClick={() => store.dispatch(actionPromise("delay1000", delay(1000)))}>
                послать в редукс шото
            </button>
...
const connector = connect(
    (reduxState) => ({status: reduxState.delay1000 && reduxState.delay1000.status}),
